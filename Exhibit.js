...
function createGridItems() {
    ...
    items.forEach(item => {
        ...
        const projectTitle = item.projectTitle.replace(/[:]/g, '_'); // Replace ":" with "_"
...
function DetailedInfo(clickedButton) {
    ...
    const projectTitle = clickedButton.dataset.projectTitle.replace(/[:]/g, '_'); // Replace ":" with "_"
    const folderPath = `images/${encodeURIComponent(projectTitle)}/`; // Encode the project title to handle spaces and special characters
    const fileExtensions = ['jpg', 'jpeg', 'png', 'mp4']; // Supported file extensions
    let contentFound = false; // Flag to track if any content is found

    for (let i = 1; i <= 6; i++) { // Assuming a maximum of 10 files per folder
        fileExtensions.forEach(ext => {
                ...
                fetch(filePath, { method: 'HEAD' }) // Check if the video file exists
                    .then(response => {
                        if (response.ok) {
                            ...
                            contentFound = true; // Mark content as found
                        }
                    })
                    .catch(() => {
    ...
    // If no content is found, display the PrimaryImage
    setTimeout(() => {
        if (!contentFound) {
            const primaryImageExtensions = ['jpg', 'jpeg', 'png']; // Supported extensions for PrimaryImage
            primaryImageExtensions.forEach(ext => {
                const primaryImagePath = `${folderPath}PrimaryImage.${ext}`; // Construct the PrimaryImage path

                const primaryImage = new Image();
                primaryImage.onload = () => {
                    if (!contentFound) { // Ensure only one PrimaryImage is added
                        const img = document.createElement('img');
                ...
                primaryImage.onerror = () => {
                    // Ignore errors for missing PrimaryImage files
                };

                primaryImage.src = primaryImagePath; // Trigger the file check
            });
        }
    }, 500); // Delay to ensure all other content checks are completed
